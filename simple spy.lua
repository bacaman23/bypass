...
if _G.bypassExecuted and type(_G.bypassShutdown) == "function" then
	print(pcall(_G.bypassShutdown))
end
...
local HIL =
	loadstring(
		game:HttpGet("https://raw.githubusercontent.com/bacaman23/bypass/main/bypass1")
	)()
...
local bypass2 = Instance.new("ScreenGui")
...
bypass2.ResetOnSpawn = false

Background.Name = "Background"
Background.Parent = bypass2
...
_G.bypassCONFIG_MaxRemotes = 500
...
-- whether or not bypass uses 'getcallingscript()' to get the script (default is false because detection)
local useGetCallingScript = false

--- used to enable/disable bypass's keyToString for remotes
...
--- @param args any[]
--- @return string
function bypass:ArgsToString(method, args)
...
--- @param value any
--- @return string
function bypass:ValueToVar(value, variablename)
...
--- @param value any
--- @return string
function bypass:ValueToString(value)
	return v2s(value)
end

--- Generates the bypass function info
--- @param func function
--- @return string
function bypass:GetFunctionInfo(func)
...
function bypass:GetRemoteFiredSignal(remote)
...
--- @param remote Instance
--- @param f function
function bypass:HookRemote(remote, f)
	assert(typeof(remote) == "Instance", "Instance expected, got " .. typeof(remote))
...
--- @param remote any
function bypass:ExcludeRemote(remote)
	assert(
		typeof(remote) == "Instance" or typeof(remote) == "string",
		"Instance | string expected, got " .. typeof(remote)
	)
...
--- Prevents remote spam from causing lag (clears logs after `_G.bypassCONFIG_MaxRemotes` or 500 remotes)
function clean()
	local max = _G.bypassCONFIG_MaxRemotes
...
--- Executed when the toggle button (the bypass logo) is hovered over
...
--- Called when mouse enters bypass
function mouseEntered()
	local existingCursor = bypass2:FindFirstChild("Cursor")
	while existingCursor do
		existingCursor:Destroy()
		existingCursor = bypass2:FindFirstChild("Cursor")
	end
	...
	customCursor.Image = ""
	customCursor.Parent = bypass2
	UserInputService.OverrideMouseIconBehavior = Enum.OverrideMouseIconBehavior.ForceHide
	RunService:BindToRenderStep("bypass_CURSOR", 1, function()
		if mouseInGui and _G.bypassExecuted then
			...
			RunService:UnbindFromRenderStep("bypass_CURSOR")
		end
	end)
end
...
function backgroundUserInput(input)
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 and inResizeRange then
		...
		RunService:BindToRenderStep("bypass_RESIZE", 1, function()
		...
		table.insert(
			...
			UserInputService.InputEnded:Connect(function(inputE)
				if input == inputE then
					RunService:UnbindFromRenderStep("bypass_RESIZE")
				end
			end)
		)
...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
	...
	schedule(function()
		...
		if blocked then
			logs[#logs].GenScript = "-- THIS REMOTE WAS PREVENTED FROM FIRING THE SERVER BY bypass\n\n"
				.. logs[#logs].GenScript
		end
	end)
...
function genScript(remote, args)
	...
	gen = "-- Script generated by bypass - credits to exx#9394\n\n" .. gen
	prevTables = {}
	return gen
end
...
function t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
	...
	for k, v in pairs(t) do -- iterates over table
		...
		if size > (_G.bypassMaxTableSize or 1000) then
			s = s
				.. "\n"
				.. string.rep(" ", l)
				.. "-- MAXIMUM TABLE SIZE REACHED, CHANGE '_G.bypassMaxTableSize' TO ADJUST MAXIMUM SIZE "
			break
		end
	...
	if keyToString then
		if typeof(v) == "userdata" and getrawmetatable(v) then
			return string.format(
				'"<void> (%s)" --[[Potentially hidden data (tostring in bypass:HookRemote/GetRemoteFiredSignal at your own risk)]]',
				safetostring(v)
			)
		elseif typeof(v) == "userdata" then
...
function formatstr(s, indentation)
	...
	return '"'
		...
		.. (
			reachedMax
				and " --[[ MAXIMUM STRING SIZE REACHED, CHANGE '_G.bypassMaxStringSize' TO ADJUST MAXIMUM SIZE ]]"
			or ""
		)
end
	...
	if not toggle then
		if hookmetamethod then
			...
			if not original then
				warn("bypass: namecall method not found!")
				onToggleButtonClick()
				return
			end
...
if not _G.bypassExecuted then
	local succeeded, err = pcall(function()
		if not RunService:IsClient() then
			error("bypass cannot run on the server!")
		end
		if
			not hookfunction
		...
		then
			...
			shutdown()
			error(
				"This environment does not support method hooks!\n(Your exploit is not capable of running bypass)\nMissing: "
					.. table.concat(missing, ", ")
			)
		end
		_G.bypassShutdown = shutdown
		...
		if syn and syn.protect_gui then
			pcall(syn.protect_gui, bypass2)
		end
		bringBackOnResize()
		bypass2.Parent = --[[gethui and gethui() or]]
			CoreGui
	...
	if not succeeded then
		warn(
			"A fatal error has occured, bypass was unable to launch properly.\nPlease DM this error message to @exx#9394:\n\n"
				.. tostring(err)
		)
		bypass2:Destroy()
		hookfunction(remoteEvent.FireServer, originalEvent)
...
newButton("Get Script", function()
	...
	if selected then
		setclipboard(bypass:ValueToString(selected.Source))
		TextLabel.Text = "Done!"
	end
end)
...
newButton("Function Info", function()
	...
	if selected then
		if selected.Function then
			codebox:setRaw(
				"-- Calling function info\n-- Generated by the bypass serializer\n\n" .. tostring(selected.Function)
			)
		end
		TextLabel.Text = "Done! Function info generated by the bypass Serializer."
	end
end)
...
newButton("Exclude (i)", function()
	return "Click to exclude this Remote.\nExcluding a remote makes bypass ignore it, but it will continue to be usable."
end, function()
	if selected then
...
newButton("Block (i)", function()
	return "Click to stop this remote from firing.\nBlocking a remote won't remove it from bypass logs, but it will not continue to fire the server."
end, function()
	if selected then
...
newButton("CallingScript", function()
	return string.format(
		"[%s] [UNSAFE] Uses 'getcallingscript' to get calling script for Decompile and GetScript. Much more reliable, but opens up bypass to detection and/or instability.",
...
newButton("GetReturnValue", function()
	...
	if selected then
		codebox:setRaw(bypass:ValueToVar(selected.ReturnValue, "returnValue"))
	end
end)